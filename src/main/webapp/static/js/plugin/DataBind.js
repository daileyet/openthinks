...
(function (factory) {
    ...
    function value(el, newVal) {
        ...
        if (['text','hidden', 'textarea', 'select-one', 'email', 'url', 'week', 'time', 'search', 'tel', 'range', 'number', 'month', 'datetime-local', 'date', 'color'].indexOf(el.type) >= 0) {
            if (isSetter) {
                el.value = newVal;
            }
            return el.value;
      ...
      if (isSetter && el.getAttribute('contenteditable')!== undefined && el.getAttribute('contenteditable') == 'true') {
          el.innerHTML = newVal;
          return el.innerHTML;
      }
      ...
      return (el.getAttribute('contenteditable')!== undefined && el.getAttribute('contenteditable') == 'true')?el.innerHTML:el.innerText;
    }
    ...
    function getEventNameForEl(el) {
        ...
        if (['text', 'textarea'].indexOf(el.type) >= 0 || (el.getAttribute('contenteditable')!== undefined && el.getAttribute('contenteditable') == 'true')) {
            return 'input';
        }
    }
