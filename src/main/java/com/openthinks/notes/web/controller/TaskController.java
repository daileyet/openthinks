...
import com.openthinks.notes.web.entity.TaskContents;
import com.openthinks.notes.web.entity.Tasks;
import com.openthinks.notes.web.service.TaskContentService;
import com.openthinks.notes.web.service.TaskService;
import com.openthinks.notes.web.utils.TaskUtils;

@Controller("/task")
public class TaskController {
	TaskService taskService = WebContexts.get().lookup(TaskService.class);
	TaskContentService contentService = WebContexts.get().lookup(TaskContentService.class);
	...
	private List<Tasks> getCurrentUserTask(WebAttributers webAttributes) {
		AuthorizedUser autnorizedUser = (AuthorizedUser) webAttributes.getAttribute(AuthorizedUser.AUTHORIZED_USER_ID,
				WebScope.SESSION);
		List<Tasks> tasks = taskService.listTask(autnorizedUser.getUser().getId());
		return tasks;
	}
	...
	public String edit(WebAttributers webAttributes) {
		...
		if (id != null) {
			...
			try {
				creatorId = task.getTaskGroup() == null ? null : task.getTaskGroup().getCreatorId();
			} catch (Exception e) {
				// ignore
			}
			if (creatorId == null) {
				if (TaskUtils.sessionUser(webAttributes).getId().equals(task.getUid())) {
					webAttributes.addAttribute("isGroupOwner", "true", WebScope.REQUEST);
				}
			} else {
				Boolean isGroupOwner = TaskUtils.sessionUser(webAttributes).getId().equals(creatorId);
				webAttributes.addAttribute("isGroupOwner", isGroupOwner.toString(), WebScope.REQUEST);
			}
		}
		List<Tasks> tasks = getCurrentUserTask(webAttributes);
		webAttributes.addAttribute("tasks", tasks, WebScope.REQUEST);
	...
	public String saveForAjax(WebAttributers webAttributes) {
		...
		Map errorMap = (Map) webAttributes.getAttribute(WebAttributers.ERROR_NAME, WebScope.REQUEST);
		String errorTem = (String) (errorMap == null ? null : errorMap.get("operation-error"));
		errorMessage = (errorMessage == null ? errorTem : errorMessage);
		if (errorMessage != null)
			return OperationJson.build().setType(OperationJson.ERROR_TYEP).setMsg(errorMessage).toString();
		else {
			Tasks tasks = (Tasks) webAttributes.getAttribute("task", WebScope.REQUEST);
			Map<String, String> others = new HashMap<String, String>();
			others.put("id", tasks.getId());
			others.put("lock", tasks.getLock());
			String response = OperationJson.build().setType(OperationJson.SUCESS_TYEP).setOther(others).toString();
			return response;
		}
	}
	...
	public String assignGroup(WebAttributers webAttributes) {
		...
		if ("Y".equals(task.getLock())// task has been locked by other person
			...
			json.setType(OperationJson.ERROR_TYEP).setMsg(
					"This task has been locked by other person first, you could not modify it.");
		} else {
			task.setTaskGroupId(groupId);
			taskService.update(task);
			json.setType(OperationJson.SUCESS_TYEP).setMsg(
					"This task has been assigned to the selected group sucessfully.");
		}
	...
	public String save(WebAttributers webAttributes) {
		...
		if (subject == null || "".equals(subject.trim())) {
			webAttributes.addError("operation-error", "The subject of task cannot be empty.", WebScope.REQUEST);
			return "task_edit.jsp";
		}
		...
		if (id != null && !"".equals(id.trim())) {// update task
			...
			if ("Y".equalsIgnoreCase(tasks.getLock()) && tasks.getLockedBy() != autnorizedUser.getUser().getId()) {
				webAttributes.addError("operation-error", "This task has been locked by user:"
						+ tasks.getLockedUser().getUserName() + " , please wait for or contact him.", WebScope.REQUEST);
			} else {
	...
	public String lock(WebAttributers webAttributes) {
		...
		if ("Y".equals(task.getLock())// task has been locked by other person
				&& !autnorizedUser.getUser().getId().equals(task.getLockedBy())) {
			json.setType(OperationJson.ERROR_TYEP).setMsg(
					"This task has been locked by other person first, you could not lock it.");
		} else {
			task.setLock("Y");
			...
			taskService.update(task);
			json.setType(OperationJson.SUCESS_TYEP).setMsg("This task has been locked by you sucessfully.");
		}
		return json.toString();
	}
	...
	public String unlock(WebAttributers webAttributes) {
		...
		if ("Y".equals(task.getLock())// task was not locked by self
			...
			.setType(OperationJson.ERROR_TYEP).setMsg(
					"This task has been locked by other person first, you could not unlock it.");
		} else {
			task.setLock("N");
			// task.setLockedBy(autnorizedUser.getUser().getId());
			// task.setLockedDate(new Date().toString());
			...
			taskService.update(task);
			json.setType(OperationJson.SUCESS_TYEP).setMsg("This task has been unlocked by you sucessfully.");
		}
		return json.toString();
	}
	...
	public String removeFormAjax(WebAttributers webAttributes) {
		...
		Map errorMap = (Map) webAttributes.getAttribute(WebAttributers.ERROR_NAME, WebScope.REQUEST);
		String errorTemp = (String) (errorMap == null ? null : errorMap.get("operation-error"));
		errorMessage = (errorMessage == null ? errorTemp : errorMessage);
		if (errorMessage != null) {
			return OperationJson.build().setType(OperationJson.ERROR_TYEP).setMsg(errorMessage).toString();
		} else {
			return OperationJson.build().setType(OperationJson.SUCESS_TYEP).toString();
		}
	}
	...
	public String remove(WebAttributers webAttributes) {
		...
		if (id != null) {
			Tasks task = taskService.find(id);
			if (task == null) {
				webAttributes.addError("operation-error", "This task can not be removed, it may has been removed.",
						WebScope.REQUEST);
				return "task_edit.jsp";
			}
			try {
			...
			webAttributes.addError("operation-error", "Please make sure you have selected a task.", WebScope.REQUEST);
		}
